...
export const DiagramRepresentation = ({
  ...
  const { toast, diagramRepresentation, selectionDialog } = value as SchemaValue;
  const {
    id,
   ...
   */
  useEffect(() => {
    if (diagramRepresentation === 'ready') {
      const selectionEvent: SelectionEvent = { type: 'SELECTION', selection };
      dispatch(selectionEvent);
    }
  }, [selection, diagramServer, diagramRepresentation, dispatch]);
  ...
  if (diagramRepresentation === 'complete') {
    content = (
